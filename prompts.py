# prompts.py
# Prompt builders and JSON schemas for Analyzer and Style-Transfer Script
...
ANALYZER_SCHEMA_EXAMPLE = {
    ...
    "narrative": [
        ...
        {"phase": "pain_point", "start_s": 1.5, "end_s": 4.0},
        {"phase": "solution", "start_s": 4.0, "end_s": 11.0},
        {"phase": "proof", "start_s": 11.0, "end_s": 16.0},
        {"phase": "cta", "start_s": 16.0, "end_s": 19.8},
    ],
    "influencer_DNA": {
        ...
        "retention_devices": ["quick cut to reveal", "on-screen counters"],
    },
    "beats": [
        {"t_s": 0.0,  "type": "cut"},
    ...
    "keyframes": [
        {"t_s": 0.0, "image_ref": "kf_01.jpg", "why": "pattern interrupt text"},
        {"t_s": 4.5, "image_ref": "kf_02.jpg", "why": "product demo close-up"},
    ...
    "transferable_patterns": [
        ...
        "show macro product close-up timed to sound hit",
...
def build_analyzer_messages(
    *,
    ...
    We pass a compact 'evidence' JSON and show the schema we want back.
    """
    evidence = {
        "platform": platform,
        "duration_s": duration_s,
        "aspect_ratio": aspect_ratio,
        "keyframes_meta": keyframes_meta,           # [{t, path}]
...
return JSON ONLY matching the schema that follows. Do not include any extra keys or commentary.
...
Rules:
- Derive timings from keyframes_meta t values and any textual cues in ocr_frames/transcript.
- Always include 'narrative' phases covering hook→pain_point→solution→proof→cta if applicable.
- 'scenes' must cover the whole runtime with contiguous start/end (no gaps/overlaps).
- 'on_screen_text' must be ≤2 short lines per scene, based on overlays or inferred intent.
- 'beats' should include any cut/impact moments you can infer from evidence.
- 'keyframes' must reference the provided keyframe file names (basename from the paths).
...
SCRIPT_SCHEMA_EXAMPLE = {
    ...
    "style_transfer": {
        ...
        "adapt": ["replace beauty close-up with earbud case macro"],
        "affordance_map": [
        ...
        "scenes": [
            {
                ...
                "on_screen_text": ["Instant connect", "ENC calls"],
                "transition_out": "hard cut",
                "sfx_or_music": "whoosh at 0.2s",
    ...
    "checklist": [
        "Safe area: on-screen text max 2 lines, big contrast",
...
- on_screen_text per scene: <= 2 lines, short, 7–10 words/line max.
- Scenes MUST be contiguous from 0s to target_runtime_s (±0.5s), no gaps/overlaps.
- Keep tone consistent with influencer DNA from the Analyzer, but adapt to brand_voice if given.
