...
from media_tools import extract_duration, grab_even_keyframes, frame_at_time, transcribe_audio, download_video_from_url
from ocr_tools import ocr_images
from validators import validate_analyzer_json, validate_script_json
from prompts import build_analyzer_prompt_with_fewshots, build_script_messages, ANALYZER_SCHEMA_EXAMPLE, ARCHETYPES, ARCHETYPE_PHASES
from product_research import research_product
from vision_tools import infer_visual_features  # NEW
...
EXEMPLARS=[{"name":"ASMR Unbox – Gadget","archetype":"SHOWCASE","mini_analysis":{"phases":[{"name":"Unbox"},{"name":"Handle/Features"},{"name":"Demo"},{"name":"Outro"}]}},
{"name":"Hands-only Beauty ASMR","archetype":"SHOWCASE","mini_analysis":{"phases":[{"name":"Handle/Features"},{"name":"Demo"}]}},
{"name":"Talky Review – Tech","archetype":"NARRATIVE","mini_analysis":{"phases":[{"name":"Hook"},{"name":"Solution"},{"name":"Proof"},{"name":"CTA"}]}},
{"name":"Tutorial – 3 Steps","archetype":"TUTORIAL","mini_analysis":{"phases":[{"name":"Step 1"},{"name":"Step 2"},{"name":"Step 3"},{"name":"Result"}]}},
{"name":"Comparison – Side by Side","archetype":"COMPARISON","mini_analysis":{"phases":[{"name":"Comparison Setup"},{"name":"Side-by-side Test"},{"name":"Result"}]}},
{"name":"Field Test – Outdoor","archetype":"TEST_DEMO","mini_analysis":{"phases":[{"name":"Setup"},{"name":"Test"},{"name":"Results"},{"name":"Takeaway"}]}}]
...
def propose_scene_candidates(duration: float, key_ts: List[float], motion_vals: List[float], threshold: float = 0.45, min_len: float = 2.0):
    if not key_ts:
        if duration >= 9.0:
...
product_url_override = st.text_input("Product page URL (optional, recommended for exact features/specs)", "")
auto_research = st.checkbox("Auto research from web + visuals", value=True)
...
run_btn = st.button("Analyze & Generate", type="primary", disabled=not ((video_url or video_file) and brand and product))
...
if run_btn:
    ...
    with st.spinner("Analyzing reference video…"):
        analyzer_prompt = build_analyzer_prompt_with_fewshots(
            ...
            keyframes_meta=keyframes_meta, ocr_frames=ocr_frames, transcript_text=transcript,
            archetype_menu=ARCHETYPES, grammar_table=ARCHETYPE_PHASES,
            fewshots=EXEMPLARS, schema_example=ANALYZER_SCHEMA_EXAMPLE,
        ...
        analysis["global_signals"].setdefault("speech_presence", "none" if sp_ratio < 0.15 else ("low" if sp_ratio < 0.35 else ("medium" if sp_ratio < 0.7 else "high")))
        ...
        if issues:
            fix_prompt = f"You produced JSON, but it has issues:\n- {'; '.join(issues[:8])}\nReturn a corrected JSON ONLY."
        ...
        if arch == "SHOWCASE" and duration >= 10 and len(phases) <= 1:
            ...
            analysis["phases"] = [{
                ...
                "what_happens": "visible action per segment", "camera_notes": "",
                "audio_notes": "silent", "on_screen_text": "", "evidence": [f"kf@{round((s['start_s']+s['end_s'])/2,2)}"]
    ...
    # Visual fallback features (from keyframe images) — used by research if needed
    vision_features = {"visible_features": [], "materials_guess": [], "avoid_claims": [], "confidence": 0.0}
    if auto_research and imgs:
        # pass the actual saved keyframe paths (from grab_even_keyframes)
        kf_paths = [k["image_path"] for k in grab_even_keyframes(str(video_path), every_s=duration/max(6, len(phases)*2), limit=8)]
        vision_features = infer_visual_features(kf_paths, provider=provider)

    # Research
    research = {"query":"", "sources":[], "claims":[], "disclaimers":[], "features":[]}
    if auto_research:
        with st.spinner("Researching the product…"):
            research = research_product(brand, product, product_url_override=product_url_override.strip(), fallback_visual=vision_features)
    ...
    with st.spinner("Generating script & shot list…"):
        script_prompt = build_script_messages(
            analyzer_json=analysis, brand=brand, product=product,
            approved_claims=final_claims or research.get("features", []),   # safe fall-back
            required_disclaimers=final_disclaimers,
            target_runtime_s=float(target_runtime), platform=platform, brand_voice={},
        )
    ...
    if pe is not None:
        try:
            pdf_bytes: bytes = pe.build_pdf(
                brand_header=pdf_brand, footer_note=pdf_footer,
                product_meta={"brand": brand, "name": product, "platform": platform},
                analysis=analysis, plan=script_json,
